...
test('WNBA totals challenger is causal, measured and remains fail-closed', () => {
  ...
  const evaluationSeasons = Object.values(report.total.per_season)
  assert.deepEqual(Object.keys(report.total.per_season), report.seasons.slice(report.burn_in.length))
  assert.equal(report.total.historical.n, evaluationSeasons.reduce((sum, season) => sum + season.n, 0))
  assert.ok(report.total.historical.n >= 1091)
  assert.ok(evaluationSeasons.every((season) => season.n > 0))
